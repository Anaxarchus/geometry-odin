...
contour_set_area :: proc(cs: [][][2]f32) -> f32 {
	total: f32
	for c in cs {
		total += polygon.polygon_get_area(c)
	}
	return total
}
...
test_polygon_area_perimeter :: proc(t: ^testing.T) {
	...
	testing.expect(t, feq(polygon.polygon_get_area(sq), 4), "area")
	testing.expect(t, feq(polygon.polygon_get_perimeter(sq), 8), "perimeter")
}
...
test_polygon_winding :: proc(t: ^testing.T) {
	...
	testing.expect(t, !polygon.polygon_is_clockwise(ccw), "ccw not clockwise")
	testing.expect(t, polygon.polygon_is_clockwise(cw), "cw clockwise")
}
...
test_polygon_convexity :: proc(t: ^testing.T) {
	...
	testing.expect(t, polygon.polygon_is_convex(sq), "square convex")
	testing.expect(t, !polygon.polygon_is_convex(l), "L not convex")
	// the L-shape has area 3 (2x2 minus a 1x1 corner)
	testing.expect(t, feq(polygon.polygon_get_area(l), 3), "L area")
}

@(test)
test_polygon_point_in :: proc(t: ^testing.T) {
	sq := [][2]f32{{0, 0}, {2, 0}, {2, 2}, {0, 2}}
	testing.expect(t, polygon.polygon_has_point(sq, [2]f32{1, 1}), "even-odd inside")
	testing.expect(
		t,
		polygon.polygon_has_point(sq, [2]f32{1, 1}, .Winding_Number),
		"winding inside",
	)
	testing.expect(t, !polygon.polygon_has_point(sq, [2]f32{3, 3}), "outside")

	// concave: the notch corner is outside, the base is inside
	l := [][2]f32{{0, 0}, {2, 0}, {2, 1}, {1, 1}, {1, 2}, {0, 2}}
	testing.expect(t, polygon.polygon_has_point(l, [2]f32{0.5, 0.5}), "L base inside")
	testing.expect(t, !polygon.polygon_has_point(l, [2]f32{1.5, 1.5}), "L notch outside")
}

@(test)
test_polygon_min_max_edges :: proc(t: ^testing.T) {
	sq := [][2]f32{{0, 0}, {2, 0}, {2, 2}, {0, 2}}
	mm := polygon.polygon_get_min_max(sq)
	testing.expect(t, veq(mm[0], [2]f32{0, 0}) && veq(mm[1], [2]f32{2, 2}), "min_max")

	edges := polygon.polygon_get_edges(sq)
	defer delete(edges)
...
test_polygon_simplify :: proc(t: ^testing.T) {
	...
	out := polygon.polygon_simplify(poly, 0.001)
	defer delete(out)
	testing.expect(t, len(out) == 4, "collinear point removed")
	testing.expect(t, feq(polygon.polygon_get_area(out), 4), "area preserved")
}
...
test_polygon_decimate :: proc(t: ^testing.T) {
	...
	out := polygon.polygon_decimate(poly, 0.1)
	defer delete(out)
...
test_polygon_triangulate :: proc(t: ^testing.T) {
	sq := [][2]f32{{0, 0}, {2, 0}, {2, 2}, {0, 2}}
	tris := polygon.polygon_triangulate(sq)
	defer delete(tris)
	...
	fast := polygon.polygon_triangulate(sq, .Fast)
	defer delete(fast)
	ftotal: f32
	for tri in fast {
		ftotal += tri_area(tri)
...
test_polygon_offset :: proc(t: ^testing.T) {
	sq := [][2]f32{{0, 0}, {2, 0}, {2, 2}, {0, 2}}
	off := polygon.polygon_offset(sq, 0.5, .Miter)
