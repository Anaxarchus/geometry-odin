...
test_rect_constructors :: proc(t: ^testing.T) {
	r := rect.from_center_size([2]f32{2, 1}, [2]f32{4, 2})
...
test_rect_accessors :: proc(t: ^testing.T) {
	r := [2][2]f32{{0, 0}, {4, 2}}
	testing.expect(t, veq(rect.size(r), [2]f32{4, 2}), "size")
	testing.expect(t, feq(rect.area(r), 8), "area")
	testing.expect(t, feq(rect.perimeter(r), 12), "perimeter")
	testing.expect(t, feq(rect.aspect(r), 2.0), "aspect")
	testing.expect(t, veq(rect.center(r), [2]f32{2, 1}), "center")

	testing.expect(t, veq(rect.corner(r, .Top_Left), [2]f32{0, 0}), "corner TL")
	testing.expect(t, veq(rect.corner(r, .Top_Right), [2]f32{4, 0}), "corner TR")
	testing.expect(t, veq(rect.corner(r, .Bottom_Left), [2]f32{0, 2}), "corner BL")
	testing.expect(t, veq(rect.corner(r, .Bottom_Right), [2]f32{4, 2}), "corner BR")

	corners := rect.corners(r)
...
test_rect_queries :: proc(t: ^testing.T) {
	r := [2][2]f32{{0, 0}, {4, 2}}
	testing.expect(t, rect.has_point(r, [2]f32{2, 1}), "has_point inside")
	testing.expect(t, rect.has_point(r, [2]f32{0, 0}), "has_point boundary")
	testing.expect(t, !rect.has_point(r, [2]f32{5, 5}), "has_point outside")

	testing.expect(
		t,
		rect.contains_rect(r, [2][2]f32{{1, 0.5}, {3, 1.5}}),
		"contains_rect true",
	)
	testing.expect(
		t,
		!rect.contains_rect(r, [2][2]f32{{-1, 0}, {3, 1}}),
		"contains_rect false",
	)
	...
	b := [2][2]f32{{1, 1}, {3, 3}}
	far := [2][2]f32{{3, 3}, {4, 4}}
	testing.expect(t, rect.intersects(a, b), "intersects true")
	testing.expect(t, !rect.intersects(a, far), "intersects false")

	ov, ok := rect.intersection(a, b)
	testing.expect(t, ok, "intersection ok")
	testing.expect(t, veq(ov[0], [2]f32{1, 1}) && veq(ov[1], [2]f32{2, 2}), "intersection rect")
	_, ok2 := rect.intersection(a, far)
	testing.expect(t, !ok2, "intersection none")

	m := rect.merge(a, b)
	testing.expect(t, veq(m[0], [2]f32{0, 0}) && veq(m[1], [2]f32{3, 3}), "merge")

	v, c := rect.nearest_vertex(r, [2]f32{-1, -1})
	testing.expect(t, veq(v, [2]f32{0, 0}) && c == .Top_Left, "nearest_vertex")

	testing.expect(t, rect.nearest_edge(r, [2]f32{2, -1}) == .Top, "nearest_edge")

	// center projects to the nearest edge (tie resolves to Top)
	testing.expect(
		t,
		veq(rect.project_to_boundary(r, [2]f32{2, 1}), [2]f32{2, 0}),
		"project center",
	)
	// outside point projects onto the right edge
	testing.expect(
		t,
		veq(rect.project_to_boundary(r, [2]f32{5, 1}), [2]f32{4, 1}),
		"project outside",
	)

	testing.expect(t, veq(rect.clamp_point(r, [2]f32{5, 5}), [2]f32{4, 2}), "clamp out")
	testing.expect(t, veq(rect.clamp_point(r, [2]f32{2, 1}), [2]f32{2, 1}), "clamp in")
}
...
test_rect_integer :: proc(t: ^testing.T) {
	ri := [2][2]int{{0, 0}, {4, 2}}
	testing.expect(t, rect.size(ri) == [2]int{4, 2}, "int size")
	testing.expect(t, rect.area(ri) == 8, "int area")
	testing.expect(t, rect.has_point(ri, [2]int{1, 1}), "int has_point")
	testing.expect(t, !rect.has_point(ri, [2]int{9, 9}), "int has_point out")

	m := rect.merge(ri, [2][2]int{{-1, -1}, {1, 1}})
