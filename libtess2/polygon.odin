...
// _is_convex reports whether a left turn is made at vertex p while marching
// prev -> p -> next. This matches the paper's convention (convex = left turn
// while marching the directed contour), so it is the correct convexity test
// whether the contour is an outer boundary (CCW) or a hole (CW).
@(private)
_is_convex :: #force_inline proc(p, prev, next: [2]f64) -> bool {
...
make_raw_offset_curve :: proc(polygon: [][2]f64, deltas: []f64, join_type: Join_Type, arc_resolution, miter_limit: f64, allocator := context.allocator) -> [][2]f64 {
    ...
    for i in 0..<C {
        ...
        is_convex := _is_convex(polygon[i], polygon[h], polygon[j])
        inner     := deltas[min(i, dC)] < 0

        // Insert a "V" straight through the original vertex at:
        //   convex  vertex + inner offset
        //   concave vertex + outer offset
        // These are the corners whose offset edges form the invalid loops that
        // the winding rule discards. The other corners (the "fans") get the
        // chosen join style.
        needs_v := (is_convex && inner) || (!is_convex && !inner)
        ...
        if needs_v {
            ...
            case .Miter:
                ...
                // Clamp sharp corners: if the miter point runs further than
                // miter_limit * offset from the vertex, fall back to a bevel.
                // miter_limit <= 0 means "no limit".
                d := max(abs(delta_in), abs(delta_out))
                within_limit := miter_limit <= 0 || linalg.length(x - polygon[i]) <= miter_limit * d
                if hit && within_limit {
                    buf[count] = x; count += 1
                } else {
                    buf[count] = p_end;   count += 1
...
// intersect_polygons returns the regions covered by at least two input polygons
// (winding rule |w| >= 2). All polygons should be CCW wound.
// NOTE: for exactly two polygons this is their intersection; for three or more
// it is "covered >= 2 times", NOT the strict n-way intersection (the tessellator
// provides no |w| >= n rule).
...
// difference_polygons subtracts the cutters from the subject.
// polygons[0] is the subject (CCW); polygons[1:] are the cutters. Returns the
// subject minus the cutters.
// NOTE: the cutter region is built with the |w| >= 2 rule, so when cutters
// overlap *each other* the result can deviate from a strict
// subject - union(cutters); it is exact when the cutters do not mutually overlap.
difference_polygons :: proc(polygons: [][][2]f64, allocator := context.allocator) -> [][][2]f64 {
    ...
    ctx, ok := begin(2, false)
        ...
        defer delete_contours(intersection)
    end(ctx)
